...
int m = atoi(argv[1]), n = atoi(argv[2]), kern_type = atoi(argv[3]), reps = atoi(argv[4]), rep;
double kernel[m*n];
int i_rank, ranks;
int param, symm;
MPI_Status status;
...
unsigned short int *bigpic = (unsigned short int *)malloc(xsize*(ysize + num)*sizeof(short int));
...
MPI_Scatterv(bigpic, sendcounts, displs, MPI_UNSIGNED_SHORT, minipic, (last-first)*xsize, MPI_UNSIGNED_SHORT, 0, MPI_COMM_WORLD);
...
if (i_rank == 0){
    ...
    write_pgm_image(final, maxval, xsize, ysize, name);
    free(name);
}
free(minipic);
free(bigpic);

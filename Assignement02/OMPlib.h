...
void weightkernel(int m, int n, double param, double * kernel){
  // per asimmetria chiedi da quale parte mettere i valori negativi
  double w;
  int i, j;

  w = (1-param)/(m * n - 1);
  printf("w is %f, f is %f\n", w, param);
  for(i = 0; i < m; i++){
    for(j = 0; j < n; j++){
      kernel[i + m * j] = w;
    }
...
void elaborate(void * ptr, int xsize, int ysize, int maxval, double * kernel, int m, int n, void * res, int procs)
{
...
#pragma omp for private(val, count, j, ii, jj)
for (i = 0; i < ysize; i++){
